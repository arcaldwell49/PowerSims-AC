#Two trials 5 time points; blocking effect for randomized order

#Function defined here
RMsim2b5 <- function(TCt1Mean=0, TCt2Mean=0, TCt3Mean=0, TCt4Mean=0, TCt5Mean=0, TEt1Mean=0, TEt2Mean=0,TEt3Mean=0, TEt4Mean=0, TEt5Mean=0,
                  r=0.5, SD=1, sample, reps=1000, label, sig = 0.05){
  
  probs <- matrix(nrow = reps, ncol = 7, dimnames = list(c(), c("RepNum", "pvalue_W", "GE_W", "L_W", 
                                                                "pvalue_B", "GE_B", "L_B")))
  prop_sig <- matrix(nrow = 1, ncol = 9, dimnames = list(c(), c("Label", "NS_w", "NS_W_prop", 
                                                                "Sig_W", "Sig_W_Prop", 
                                                                "NS_B", "NS_B_prop", 
                                                                "Sig_B", "Sig_B_Prop")))
  ...
  for (i in seq(1,reps,1)) {
    ...
    data_longC$time <- NA
    data_longC$time <- ifelse(data_longC$point == 'V1' | data_longC$point == 'V6', 1,
                              ifelse(data_longC$point == 'V2' | data_longC$point == 'V7', 2,
    ...
    data_longC$trial <- NA
    data_longC$trial <- ifelse(data_longC$point == "V1" | data_longC$point == "V2" | data_longC$point == "V3" |data_longC$point == "V4" |data_longC$point == "V5", 
                               "CON", "EXP")
    ...
    data_longE <- tidyr::gather(dataE, point, measure, V1:V10, factor_key=TRUE)
    data_longE$time <- NA
    data_longE$time <- ifelse(data_longE$point == 'V1' | data_longE$point == 'V6', 1,
                              ifelse(data_longE$point == 'V2' | data_longE$point == 'V7', 2,
    ...
    data_longE$trial <- NA
    data_longE$trial <- ifelse(data_longE$point == "V1" | data_longE$point == "V2" | data_longE$point == "V3" |data_longE$point == "V4" |data_longE$point == "V5", 
                               "EXP", "CON")
    ...
    modelRMb <- aov(measure ~ order*trial*time + Error(id/(trial*time)), data=datarun)
    modelRMw <- aov(measure ~ trial*time + Error(id/(trial*time)), data=datarun)
    
    modW <- lapply(modelRMw, broom::tidy)
    modB <- lapply(modelRMb, broom::tidy)
    
    modW <- modW$`id:trial:time`
    modB <- modB$`id:trial:time`
    
    probs[i,2] <- modW$p.value[1]
    ifelse(modW$p.value[1] < sig, probs[i,3] <- 0, probs[i,3] <- 1)
    ifelse(probs[i,3] == 0, probs[i,4] <- 1, probs[i,4] <- 0)
    
    
    probs[i,5] <- modB$p.value[1]
    ifelse(modB$p.value[1] < sig, probs[i,6] <- 0, probs[i,6] <- 1)
